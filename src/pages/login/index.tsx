...
import { IS_TEST, LOGIN_SECTION_TIME, SMS_RESEND_TIME } from '@/settings/config';
...
const ResendButton = memo(({ onClick }: { onClick: () => void }) => {
  ...
  useEffect(() => {
    setCount({ top: 0 }, { duration: SMS_RESEND_TIME, easing: Bezier.linear });
  }, []);
...
const Login = memo(() => {
  ...
  useEffect(() => {
    ...
    if (storage) {
      ...
      const isValid = data && timestamp < LOGIN_SECTION_TIME;
